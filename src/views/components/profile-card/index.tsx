...
import {nip05, nip19} from 'util/nostr-utils';
...
const ProfileCard = (props: { profile: Profile, pub: string, onDM: () => void }) => {
    ...
    return <Paper sx={{textAlign: 'center', p: '20px'}}>
        ...
        {((typeof profile.name === 'string' && profile.name) || nip05Verified) && (<Box sx={{
            ...
            {typeof profile.name === 'string' ? truncate(profile.name, 60) : ''}
        ...
        {typeof profile.about === 'string' && profile.about && (
            <>
                ...
                <Box sx={{
                    ...
                    fontSize: '0.9em',
                }}>{typeof profile.about === 'string' ? truncate(profile.about, 360) : ''}</Box>
            </>
        )}
