...
const MentionList = forwardRef<MentionListRef, MentionListProps>((props, ref) => {
    ...
    const items = useMemo(() => suggestionProfiles.filter(x => typeof x.name === 'string' && x.name)
        .filter(x => typeof x.name === 'string' && x.name.toLowerCase().indexOf(query.toLowerCase()) > -1).slice(0, 10).map(x => ({
            name: typeof x.name === 'string' ? x.name : '',
            id: x.creator,
            picture: typeof x.picture === 'string' ? x.picture : ''
