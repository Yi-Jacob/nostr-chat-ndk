...
const ChannelPublicPage = (props: RouteComponentProps) => {
    ...
    const raven = useMemo(() => {
        ...
        // We'll use a simple approach to fetch channel data
        return {
            fetchChannel: async (channelId: string) => {
                try {
                    // Simple channel fetch without authentication
                    const response = await fetch(`https://api.nostr.band/v0/channel/${channelId}`);
                    if (response.ok) {
