...
import NDKRaven, {RavenEvents} from 'raven/ndk-raven';
import {
    Channel,
    ChannelUpdate,
    DirectMessage,
...
import {nip04, nip19} from 'util/nostr-utils';
...
const RavenProvider = (props: { children: React.ReactNode }) => {
    ...
    const raven = useMemo(() => {
        if (!ndk || !keys) return null;
        return new NDKRaven(ndk, keys.priv, keys.pub);
    }, [ndk, keys]);
    ...
    useEffect(() => {
        if ((keys?.priv === 'nip07' || keys?.priv === 'none') && muteList.encrypted) {
            ...
            if (keys.priv === 'none' && tempPriv) {
                ...
                promise = (window as any).nostr?.nip04.decrypt(keys.pub, muteList.encrypted);
            }

            promise?.then((e: any) => JSON.parse(e)).then((resp: any) => {
                setMuteList({
        ...
        if ((keys?.priv === 'nip07' || keys?.priv === 'none') && directMessage) {
            ...
            if (toDecrypt) {
                ...
                if (keys.priv === 'none' && tempPriv) {
                    ...
                    promise = (window as any).nostr?.nip04.decrypt(toDecrypt.peer, toDecrypt.content);
                }
